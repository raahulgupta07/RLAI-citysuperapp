...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (!cleanUrl) return json({ error: 'Invalid URL' }, { status: 400 });

  const maxOrder = db.select({ max: apps.sort_order }).from(apps).get();
  const nextOrder = (maxOrder?.max || 0) + 1;

  const result = db.insert(apps).values({
    name: sanitizeText(name), slug: (slug || name).toLowerCase().replace(/[^a-z0-9]+/g, '-'),
    url: cleanUrl, description: sanitizeText(description || ''), category_id, launch_mode: launch_mode || 'redirect',
